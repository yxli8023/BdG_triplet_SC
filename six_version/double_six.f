...
!      Article:Phys.Rev.B 96,184508
      ...
      ! lapack parameter 
      integer::lda = N
      integer,parameter::lwmax = 2*48*48*4+(48*48*4)**2
      real,allocatable::w(:)
...
c===============================Parameter Setting=======================
      ...
      h = 0.0 ! zeeman  field
      mu = -4.0  ! chemical potential
      V = 0    ! point potential
      t0 = 1.0   ! hopping energy
      lambda = 0   ! couple energy
      B = 2*phi0/(48*48)  ! magnetic field
      Kb = 0.1    !
      T = 0.1     ! temperature
...
!--------------------------- open file---------------------------------------------------------
!      open(10,file='H.dat')
      open(11,file='eig-value-double.dat')
...
!=========================================== Energy Gap function ==============================================================
      ...
      complex function delta(x,eig_value,eig_vector)
      ! x,y point position
...
!----------------------------------------positive energy ---------------------------------------------
            do m = 1,48*48               !   (1,3)block
                  Ham(m,len2*2+m) = delta_init  ! C-up*C-down
                  Ham(len2*2+m,m) = delta_init  ! E-up*E-down
            end do
!            do m = 1,48*48               !   (3,1)block
!                  Ham(len2*2+m,m) = delta_init  ! C-up*C-down
!                  Ham(m,len2*2+m) = delta_init  ! E-up*E-down
!            end do
            
!-----------------------------------------negative energy ---------------------------------------------

            do m = 1,48*48               !   (2,4)block
                  Ham(len2+m,len2*3+m) = -delta_init  ! C-up*C-down
                  Ham(len2*3+m,len2+m) = -delta_init  ! E-up*E-down
            end do
 !           do m = 1,48*48               !   (4,2)block
 !                 Ham(len2*3+m,len2+m) = -delta_init  ! C-up*C-down
 !                 Ham(len2+m,len2*3+m) = -delta_init  ! E-up*E-down
 !           end do
      end if
      return
      end subroutine            
...
!     right hopping is positive       left hopping is negatie 
      do l = 1,48                 ! 最后一列向右跳跃没有考虑到，但是向左跳跃考虑率到了
...
!     up hopping is positive               down hopping is negative
      do m = 1,len1   ! (1,1)block
            ...
            Ham(m,48*47+m) = -t0*conjg(phi(m*48))  !
            Ham(48*47+m,m) = -t0*phi(48*m)  ! last row hopping t first row
            Ham(48*47+m,48*47+m-len1) = -t0 !last row hopping to down direction
      end do      
...
!------------------------ first coloum left-direction hopping (include last coloum hopping towards first cloum cause of matrix symmetry consider)----
      do m = 1,48   ! (1,1) block
            Ham(m*48,m*48-47) = -t0*phi(m)   ! last point hopping to right
            Ham(m*48,m*48-1) = -t0*conjg(phi(m))! last point hopping to left
            Ham(1+(m-1)*48,1+(m-1)*48+1) = -t0*phi(m)! first point hopping to right
            Ham(1+(m-1)*48,m*48) = -t0*conjg(phi(m))! first point hopping to left
      end do            
      do m = 1,48   ! (2,2) block
...
! ------------------------------- negative energy couple (4,3) block------------------------------------
      do m = 2+(l-1)*48,48*l-1
          ...
          Ham(len2*3+m,len2*2+m-1) = -conjg(-image*lambda*conjg(phi(l)))  ! left
      end do
      end do
! XXXX--------------------------------------------------------------------------------------------------------------
...
! 
      do m = 1,48       ! (1,2)block
            ...
            Ham(m,len2+48*47+m) = -lambda*conjg(phi(m*48)) ! First row  
            Ham(48*47+m,len2+m) = lambda*phi(m*48)!Last row  
            Ham(48*47+m,len2+48*47-len1+m) = -lambda  
      end do
      do m = 1,48       ! (2,1)block
...
! ------------------------------------ negative energy ----------------------------------------------------------
      do m = 1,48       ! (3,4)block
!            Ham(len2*2+m,len2*2+len2+48*47+m) = lambda*conjg(phi(m*48))
...
!                        stop
      ...
      integer m
      real d
      open(18,file='eig-err-double.dat')
      do m = 1,len2*4
...
c--------------------------- point potential element setting complete-----------
      ...
      subroutine potential(pos)
      use param
      integer pos
      Ham(pos,pos)=Ham(pos,pos)+V ! position at spin-up oriention   (1,1)block
      Ham(pos+len2,pos+len2)=Ham(pos+len2,pos+len2)+V ! position at spin-down oriention   (2,2) block
      Ham(pos+len2*2,pos+len2*2)=Ham(pos+len2*2,pos+len2*2)+V    ! (3,3)block
      Ham(pos+len2*3,len2+len2*3)=Ham(pos+len2*3,len2+len2*3)+V       !(4,4)block
      return
