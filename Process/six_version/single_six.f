...
!     四个格点可以形成一个闭合的回路,逆时针方向,回路所包含的位相为B,顺时针方向所包含的位相为-B
!     48*48的点阵会形成47*47个回路，在考虑到y与x方向的周期性问题之后，每个方向再加上47个回路
!     共有47*47+2*47=2303个回路，少于48*48=2304，正好通过整体格点四个顶角位置，可以形成一个回路
...
!     up hopping is positive               down hopping is negative
      do m = 1,48   ! (1,1)block
            ...
            Ham(m,48*47+m) = -t0*conjg(phi(48*m))  ! 第一行电子向下跳跃
...
!           (2,2)block
            ...
            Ham(len2+m,len2+48*47+m) = -t0*conjg(phi(48*m))  ! 
...
!           (3,3) block
            ...
            Ham(len2*2+m,len2*2+48*47+m) = t0*conjg(phi(48*m))  !
...
!           (4,4)block
            ...
            Ham(len2*3+m,len2*3+48*47+m) = t0*conjg(phi(48*m))  ! 
...
!                        stop
      ...
      complex bb
      do m = 1,48*47*4
            bb = Ham(m,m+1)+Ham(m+1+len1,m+len1)
            write(22,*)bb
      end do
      return
      end subroutine f2
