...
!      write(*,*)"(1,1)->(1,48)",Ham(1,48)
!      write(*,*)"(1,48)->(1,1)",Ham(48,1)
!      write(*,*)"(1,1)->(48,1)",Ham(1,48*47+1)
!      write(*,*)"(48,1)->(1,1)",Ham(48*47+1,1)
!      write(*,*)"(48,1)->(48,48)",Ham(48*47+1,48*48)
!      write(*,*)"(48,1)->(1,1)",Ham(48*48,48*47+1)
!      write(*,*)"(48,48)->(1,48)",Ham(48*48,48)
!      write(*,*)"(48,1)->(1,1)",Ham(48,48*48)
...
!     up hopping is positive               down hopping is negative
      ...
      if (mod(m,2) .eq. 0)then
            ...
            Ham(m,48*47+m) = -t0*phi(l*48)  ! 第一行电子向下跳跃
            Ham(48*47+m,m) = -t0*conjg(phi(48*l))  ! 最后一行电子向上跳跃，人为的引入一个位相，该位相与x方向坐标有关
...
!      do m = 1,len1   ! (2,2)block
            ...
            Ham(len2+m,len2+48*47+m) = -t0*phi(l*48)  ! 
            Ham(len2+48*47+m,len2+m) = -t0*conjg(phi(48*l))  ! last row hopping t first row
...
!      do m = 1,len1   ! (3,3)block
            ...
            Ham(len2*2+m,len2*2+48*47+m) = t0*phi(l*48)  !
            Ham(len2*2+48*47+m,len2*2+m) = t0*conjg(phi(48*l))  ! last row hopping t first row
...
!      do m = 1,len1   ! (4,4)block
            ...
            Ham(len2*3+m,len2*3+48*47+m) = t0*phi(l*48)  ! 
            Ham(len2*3+48*47+m,len2*3+m) = t0*conjg(phi(48*l))  ! last row hopping t first row
...
! 
      do m = 1,48       ! (1,2)block
            ...
            Ham(m,len2+48*47+m) = -lambda*phi(l*48) ! 第一行向下进行耦合(直接耦合到最后一行) 
            Ham(48*47+m,len2+m) = lambda*conjg(phi(l*48))! 最后一行向上耦合(直接耦合到第一行) 
            Ham(48*47+m,len2+48*47-len1+m) = -lambda  ! 最后一行电子向下进行耦合
!      end do
!      do m = 1,48       ! (2,1)block
            Ham(len2+m,m+len1) = -lambda
            Ham(len2+m,48*47+m) = lambda*phi(l*48)
            Ham(len2+m+48*47,m) = -lambda*conjg(phi(l*48))
...
!      do m = 1,48       ! (3,4)block
     ...
     &                        -conjg(lambda*phi(l*48))
            Ham(len2*2+m+48*47,len2*3+m) =
     &                        -conjg(-lambda*conjg(phi(l*48)))
