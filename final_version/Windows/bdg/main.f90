...
!===============================Parameter Setting=======================
      ...
      xp = 1
      yp = 1
      open(11,file='param.dat',status="unknown")
      write(11,*)"h=",h
      write(11,*)"V=",V
      write(11,*)"T=",T
      write(11,*)"xp=",xp
...
!--------------------------- open file------------------------
      open(11,file='eigenvalue.dat',status="unknown")
      do m = 1,N
...
!------------------------ first coloum left-direction hopping (include last coloum hopping towards first cloum cause of matrix symmetry consider)----
      ...
      ! (3,3) block
            ...
            Ham(len2*2+1+(m-1)*xn,len2*2+1+(m-1)*xn+1) =  &
                  t0*conjg(phi(m))! first point hopping to right
      ...
      ! (4,4) block
            ...
            Ham(len2*3+1+(m-1)*xn,len2*3+1+(m-1)*xn+1) = &
             t0*conjg(phi(m))! first point hopping to right
...
!---------------------------------------negative energy---------------------------------
      !  (3,4)block
            Ham(len2*2+m*xn,len2*3+m*xn-(xn-1)) = &
                             image*lambda*conjg(phi(m))
            Ham(len2*2+m*xn,len2*3+m*xn-1) =  &
                             -image*lambda*(phi(m))! 增加一层会使自旋取向改变一次
            Ham(len2*2+m*xn-(xn-1),len2*3+m*xn-(xn-1)+1) = &
                             image*lambda*conjg(phi(m))
            Ham(len2*2+m*xn-(xn-1),len2*3+m*xn) =  &
                             -image*lambda*(phi(m))
        !  (4,3)block
            Ham(len2*3+m*xn,len2*2+m*xn-(xn-1)) = &
                             image*lambda*conjg(phi(m))
            Ham(len2*3+m*xn,len2*2+m*xn-1) =  &
                             -image*lambda*(phi(m)) ! 增加一层会使自旋取向改变一次
            Ham(len2*3+m*xn-(xn-1),len2*2+m*xn-(xn-1)+1) = &
                             image*lambda*conjg(phi(m))
            Ham(len2*3+m*xn-(xn-1),len2*2+m*xn) =  &
                             -image*lambda*(phi(m))
      end do           
...
! ------------------------------------ negative energy ----------------------------
      ! (3,4)block
            Ham(len2*2+m,len2*3+m+len1) = &
                             lambda
            Ham(len2*2+m,len2*3+xn*(yn-1)+m) = &
                             -lambda*conjg(phi(m*yn))
            Ham(len2*2+m+xn*(yn-1),len2*3+m) = &
                            lambda*(phi(m*yn))
            Ham(len2*2+m+xn*(yn-1),len2*3+m+xn*(yn-1)-len1) = &
                             -lambda
      ! (4,3)block
            Ham(len2*3+m,len2*2+m+len1) = &
                             -lambda
            Ham(len2*3+m,len2*2+xn*(yn-1)+m) = &
                             lambda*conjg(phi(m*yn))
            Ham(len2*3+m+xn*(yn-1),len2*2+m) = &
                             -lambda*(phi(m*yn))
            Ham(len2*3+m+xn*(yn-1),len2*2+m+xn*(yn-1)-len1) = &
                             lambda
      end do
!YYYY--------------------------------------------------------------------
      return
...
!=============================================================
      ...
      integer num     
      complex,external::delta
      open(12,file="order.dat",status="unknown")
      do m = 1,xn
            do l = 1,yn
                  i = (m-1)*yn+l
      ...
      if (mod(num,1) .eq. 0)then
            open(14,file='order_module.dat',status="unknown")
            do m = 1,xn
                  do l = 1,yn
...
!===========================================================================
      ...
      ref = 3                    
      do while(ref .gt. 1)
      open(24,file="count.dat",status="unknown")
      open(25,file="check.dat",status="unknown")
      open(22,file="real.dat",status="unknown")
      open(23,file="imag.dat",status="unknown")
      open(26,file="delerr.dat",status="unknown")
            ref = 0
      !do while(minval(abs(del_err))>eps)
            ...
            do l=1,yn
                  ...
                  if (real(del_err(m,l)) .ne. 0) then
                        write(22,*)real(del_err(m,l))
                  end if
                  if (imag(del_err(m,l)) .ne. 0) then
                        write(23,*)imag(del_err(m,l))
                  end if
                  write(26,*)del_err(m,l)
            end do
            end do
            close(22)
            close(23)
...
!==========================================================
      ...
      z1 = 0
      z2 = 0
      open(11,file="gamma1.dat",status="unknown")
      open(12,file="gamma2.dat",status="unknown")
      do m = 1,xn*yn
