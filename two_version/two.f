...
!     ---------------------- positive energy  -----------------------------------------
      do m = 1,48*48    ! (1,1)block   spin-up
            Ham(m,m+1) = t0  ! (i--->i+1)  正向跳跃
            Ham(m+1,m) = t0  ! (i+1--->i) 相当于考虑了反向跳跃
      end do
      do m = len2+1,len2+48*48   ! (2,2)block   spin-down
            Ham(m,m+1) = t0
            Ham(m+1,m) = t0
      end do
!     --------------------------negative energy ------------------------
      do m = len2*2+1,len2*2+48*48    ! (3,3) block
            Ham(m,m+1) = -t0
            Ham(m+1,m) = -t0
      end do
...
!     -- 上面的循环并未考虑到最后一列的点向右跳跃应该回到最左端，下面进行修改------------------------
      do m = 1,48   !  (1,1) block
            Ham(48*m,48*m+1) = 0
            Ham(48*m,1+48*(m-1)) = t0
      end do

      do m = 1,48   !  (2,2) block
            Ham(len2+48*m,len2+48*m+1) = 0
            Ham(len2+48*m,1+len2+48*(m-1)) = t0
      end do

      do m = 1,48   !   (3,3) block
            Ham(len2*2+48*m,len2*2+48*m+1) = 0
            Ham(len2*2+48*m,1+len2*2+48*(m-1)) = -t0
      end do
 
      do m = 1,47   !   (4,4)  block
            Ham(len2*3+48*m,len2*3+48*m+1) = 0
            Ham(len2*3+48*m,1+len2*3+48*(m-1)) = -t0
      end do
...
! ------------- 暂时没有考虑第一行向下跳跃时的位相问题------------------------------------------------------------------------------------------
      do m = 1,len1   ! (1,1)block
            Ham(m,48*47+m) = t0  ! 第一行下最后一行跳跃
            Ham(48*47+m,m) = t0  ! 该项为上一项的对称相,也就代表着从最后一行向第一行跳跃的问题
      end do      
...
!------------------------ first coloum left-direction hopping (include last coloum hopping towards first cloum cause of matrix symmetry consider)----
      do m = 1,48   ! (1,1) block
            Ham(1+(m-1)*48,m*48) = t0
            Ham(m*48,1+(m-1)*48) = t0
      end do            
      do m = 1,48   ! (2,2) block
            Ham(len2+1+len2+(m-1)*48,m*48) = t0
            Ham(len2+m*48,1+len2+(m-1)*48) = t0
      end do            
      do m = 1,48   ! (3,3) block
            Ham(len2*2+1+len2*2+(m-1)*48,m*48) = -t0
            Ham(len2*2+m*48,len2*2+1+(m-1)*48) = -t0
      end do            
      do m = 1,48   ! (4,4) block
            Ham(1+len2*3+(m-1)*48,m*48) = -t0
            Ham(len2*3+m*48,1+len2*3+(m-1)*48) = -t0
      end do            
