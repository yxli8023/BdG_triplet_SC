...
c                 hopping energy    跳跃是同一块中的操作
            ...
            Hamiltonian(1,48) = Ht  ! left hopping 向右跳跃时利用周期性边界条件跳跃到最右端
            ! up-down hopping
            Hamiltonian(1,48*1+1) = Ht ! 向上跳跃时加一层
            Hamiltonian(1,48*47+1) = Ht! 向下跳跃时通过周期性边界条件到最上端的一层
c                 spin-orbital couple    耦合作用涉及到不同块之间的操作，一般是跳跃到下一个块去耦合作用  每个块之间间隔大小为48*48
            ! left-right couple
            Hamiltonian(1,len2+2) = Hso ! right  代表第2个位置上的自旋向下产生算符  48*48=len2 增加了一个块
            Hamiltonian(1,len2+48) = Hso ! left   (1,1)格点位置向左方耦合为最右端的格点  一个len2增加一个块
            ! up-down couple
            Hamiltonian(1,len2+48*1+1) = Hso !up  上下跳跃的时候x方向的坐标是相同的，故为+1   而+48*1代表跳跃了一层
...
c                 spin-orbital couple
            ...
            Hamiltonian(m,len2+m-1) = Hso ! left  对于(2,1)-->(47,1)他们属于中间位置，左右方向都是有确定的格点，不会涉及到周期性边界的问题
            ! up-down couple
            Hamiltonian(m,len2+48*1+m) = Hso !up
            Hamiltonian(m,len2+48*47+m) = Hso ! down (2,1)-->(47,1)在向下耦合时，遇到的边界问题相同，都是周期性边界条纹
c                superconductor pairing energy
...
c                 hopping energy
            ! left-right hopping
            Hamiltonian(48,1) = Ht  ! right hopping  周期性边界说明48右端是(1,1)
            Hamiltonian(48,47) = Ht  ! left hopping  左端是(47,1)格点位
            ! up-down hopping
            Hamiltonian(48,48*1+48) = Ht ! up   
            Hamiltonian(48,48*47+48) = Ht ! down   向下跳跃时继续遇到周期性边界条件，连接到最上面的一行
c                 spin-orbital couple
            ! left-right couple
            Hamiltonian(48,len2+1) = Hso ! right 向右跳跃会遇到周期性边界条件，连接到(1,1)
            Hamiltonian(48,len2+48-1) = Hso ! left  一个len2跳跃一块  +48 到跳跃块的对应位置上
            ! up-down couple
            Hamiltonian(48,len2+48*1+48) = Hso !up
            Hamiltonian(48,len2+48+48*47) = Hso ! down  向下进行耦合时满足周期性边界条件---->(48,48)
c                superconductor pairing energy
...
c                 hopping energy
            ! left-right hopping
            Hamiltonian(1+(m-1)*48,(m-1)*48+1+1) = Ht  ! right hopping
            Hamiltonian(1+(m-1)*48,48+(m-1)*48) = Ht  ! left hopping 向左方向跳跃，直接跳跃到右边界
            ! up-down hopping
            Hamiltonian(1+(m-1)*48,1+(m-1)*48+48) = Ht ! up 向上跳跃增加一层   层的增加为48  块的增加为48*48
            Hamiltonian(1+(m-1)*48,1+(m-1)*48-48) = Ht ! down 向下跳跃减少一层
c                 spin-orbital couple
            ! left-right couple
            Hamiltonian(1+(m-1)*48,len2+1+(m-1)*48+1) = Hso ! right  对于耦合 首先增加一个块，然后再从对角位置向左或者向右跳跃
            Hamiltonian(1+(m-1)*48,len2+(m-1)*48+48) = Hso ! left 对于左边界上的点，向左跳跃碰到周期性边界条件，直接增加到左右边就可以
            ! up-down couple
            Hamiltonian(1+(m-1)*48,len2+1+(m-1)*48+48*1) = Hso !up 在原来的位置上直接增加一层即可
            Hamiltonian(1+(m-1)*48,len2+1+(m-1)*48-48*1) = Hso ! down (2,1)->(47,1)属于中间位置，上下跳跃或者耦合不会出现周期性边界条件的问题 
c                superconductor pairing energy
...
c                 hopping energy
            ! left-right hopping
            Hamiltonian(48*47+1,48*47+1+1) = Ht  ! right hopping
            Hamiltonian(48*47+1,48*47+48) = Ht  ! left hopping
            ! up-down hopping
            Hamiltonian(48*47+1,1) = Ht ! up hopping
            Hamiltonian(48*47+1,48*47+1-48) = Ht ! down hopping 
c                 spin-orbital couple
            ! left-right couple
            Hamiltonian(48*47+1,len2+48*47+1+1) = Hso ! right
            Hamiltonian(48*47+1,len2+48*48) = Hso ! left
            ! up-down couple
            Hamiltonian(48*47+1,len2+1) = Hso !up
            Hamiltonian(48*47+1,len2+48*47+1-48) = Hso ! down !cause of periodic first row and last row are nearest-neighbor
c                superconductor pairing energy
...
c                 hopping energy
            ...
            Hamiltonian(m,m-1) = Ht  ! left hopping
            ! up-down hopping
            Hamiltonian(m,m-48*47) = Ht ! up hopping  上边界向上跳跃是跳跃到下边界
            Hamiltonian(m,m-48*1) = Ht ! down hopping  上边界向下跳跃是减去一层
c                 spin-orbital couple
            ! left-right couple
            Hamiltonian(m,len2+m+1) = Hso ! right
...
c                superconductor pairing energy
            Hamiltonian(m,len2*2+m) = del 
      end do
c====================================================(48,48)============================================
c                 hopping energy
            ! left-right hopping
            Hamiltonian(48*48,48*47+1) = Ht  ! right hopping 最右端的点向右跳跃满足周期性边界条件跳跃到最左端
            Hamiltonian(48*48,48*48-1) = Ht  ! left hopping
            ! up-down hopping
            Hamiltonian(48*48,48) = Ht ! up hopping 向上跳跃到(1,48)
            Hamiltonian(48*48,48*47) = Ht ! down hopping 向下跳跃到(47,48)
c                 spin-orbital couple
            ! left-right couple
            Hamiltonian(48*48,len2+48*47+1) = Hso ! right
            Hamiltonian(48*48,len2+48*48-1) = Hso ! left
            ! up-down couple
            Hamiltonian(48*48,len2+48) = Hso !up
            Hamiltonian(48*48,len2+48*47) = Hso ! down !cause of periodic first row and last row are nearest-neighbor
c                superconductor pairing energy
...
c           hopping energy
            ! left-right hopping
            Hamiltonian(48*m,(m-1)*48+1) = Ht  ! right hopping  最右端的点通过周期性边界条件跳跃到最左端
...
c                 spin-orbital couple    //  While hopping to the next block
            ! left-right couple
            Hamiltonian(48*m,len2+(m-1)*48+1) = Hso ! right
            Hamiltonian(48*m,len2+48*m-1) = Hso ! left
            ! up-down couple
            Hamiltonian(48*m,len2+48*(m+1)) = Hso !up
            Hamiltonian(48*m,len2+48*(m-1)) = Hso ! down !cause of periodic first row and last row are nearest-neighbor
c                superconductor pairing energy
            Hamiltonian(48*m,len2*2+48*m) = del ! conjg is complex conjugate function
      end do
...
c       Hopping energy
      ! left-right hop energy (x-direction)
      do l = 2,47
      do m=len2+48+2,len2+48*l-1 ! (2400-->4559)second block // decrease 1 is ensure avoid boundary point first
...
c       Hopping energy
      ! left-right hop energy (x-direction)
      do l=2,47
      do m=2*len2+48+2,2*len2+48*l-1  ! (4658-->6863)
            Hamiltonian(m,m-1) = Ht
            Hamiltonian(m,m+1) = Ht 
c     ! up-down hop energy(y-driection)
...
c       Hopping energy
      ! left-right hop energy (x-direction)
      do l = 2,47
      do m=3*len+48+2,3*len+48*l-1 !(6962-->9167)
            Hamiltonian(m,m-1) = Ht
            Hamiltonian(m,m+1) = Ht 
c     ! up-down hop energy(y-driection)
            Hamiltonian(m,48*1+m) = Ht ! up layer
